...
if [[ $# -gt 1 || ( $# -eq 1 && "$1" != "--resume" ) ]]; then
    echo "Usage: bash scripts/benchmark_offline.sh [output_dir] [--resume]" >&2
    exit 2
fi
test -f configs/benchmark.yaml
...
PY
exec python -u src/benchmark_scaffolds.py --config configs/benchmark.yaml --output-dir "$OUTPUT" "$@"
